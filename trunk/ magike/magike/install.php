...
include("./include/system/api.php");
...
<head>
<meta http-equiv="Content-Type" content="text/html; charset={$global.charset}" />
<title>Magike安装程序</title>
<style>
body{background:#FFF;margin:0;padding:0}
#element{background:#666;margin:0 auto;padding:10px;width:500px;height:460px;border:2px solid #333;border-top:none;color:#FFF;font-weight:bold;text-align:right}
#element h2{color:#FFFFEE;padding:15px;margin:0;font-family:'Georgia', 'Times New Roman', Times, serif;border-bottom:1px solid #FFFFEE}
#element input{border:1px solid #999;background:#333;height:25px;font-size:15pt;width:350px;color:#FFF;font-weight:bold;}
#element #show{border-top:1px solid #FFFFEE;color:#FFFFEE;text-align:left}
...
<form method="post" action="<?php echo install_get_siteurl()."/install.php?step=2" ?>"/>
	...
	<p>数据库用户: <input type="text" name="user" /></p>
	<p>数据库密码: <input type="text" name="password" /></p>
	<p>数据库名称: <input type="text" name="dbname" /></p>
	<p>数据表前缀: <input type="text" name="one" value="mg_" /></p>
	<div id="show">
	<p>请将根目录和 ./data 目录的权限修改为 777</p>
	<p>请填写Mysql数据库相关资料,我们将对您的数据库进行初始化</p>
	<p>如果您不知道以上资料,请跟您的主机提供商联系</p>
<?php if(!is_writeable(".") || !is_writeable("./data")){ ?>	<p>我们无法进行下一步,因为您的 
	<?php if(!is_writeable(".")){ ?>根目录<?php } ?> 
		<?php if(!is_writeable("./data")){ ?>./data<?php } ?> 不可写入,请调整后刷新此页</p><?php } ?>
	<p><input type="submit" class="button" value="下一步" <?php if(!is_writeable(".") || !is_writeable("./data")){ ?>disabled=disabled style="color:#aaa" <?php } ?> /></p>
	</div>
</form>
</div>
<?php
...
//创建数据目录
if(!is_dir("./data/option")) mkdirs("./data/option");
if(!is_dir("./data/upload")) mkdirs("./data/upload");
build_cache("./data/option/config.php",create_xml_message("config",$config,true));
...
@mysql_select_db($config['dbname'], $dblink) or die("对不起,您指定的数据库并不存在");
		...
		$sql = mysql_query("
		INSERT INTO `{$config['one']}posts` VALUES (2 , '关于我们', 'about', '2006-06-12 00:00:00', '2006-06-28 00:00:00', '', 'publish', 'page', '这是一个测试页面', '', 1, 1, 'KsQi', '', 0, 'sun')
		") or die(mysql_error());
...
<magike-option>
	<option name=\"global\">
		...
		<value name=\"blogname\"><![CDATA[{$_POST['blogname']}]]></value>
...
<div id="element" style="height:300px">
	...
	<p>强烈建议您登录后立刻修改它们,根目录下的install.php还没有删除,请立刻删除</p>
